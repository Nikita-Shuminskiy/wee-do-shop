...
export const test = {
	...
	thursday: "08:30 - 23:50",
	friday: "10:00 - 23:50",
	saturday: "Closed",
}
export function isCurrentTimeWorkStoreRange(
	...
	const current_hour = String(getHours(date)).padStart(2, "0")
	const current_minute = String(getMinutes(date)).padStart(2, "0")
	...
	const [start_current_work_stores_time, end_current_work_stores_time] =
		current_day_time_work_store?.split(" - ")
	...
	const is_current_day_closed =
		!current_day_time_work_store || current_day_time_work_store === "Closed"
	...
	const curr_work_time_start = Number(
		start_current_work_stores_hour + start_current_work_stores_minute
	)
	...
	if (
		/*(!is_prev_day_after_midnight && (current_time < curr_work_time_start || current_time > curr_work_time_end))*/
		...
		(!is_prev_day_after_midnight &&
			(current_time < curr_work_time_start ||
				(current_time > curr_work_time_end && !is_current_day_after_midnight))) ||
		(is_prev_day_after_midnight &&
			current_time > prev_work_time_end &&
			(current_time < curr_work_time_start ||
				(current_time > curr_work_time_end && !is_current_day_after_midnight))) ||
